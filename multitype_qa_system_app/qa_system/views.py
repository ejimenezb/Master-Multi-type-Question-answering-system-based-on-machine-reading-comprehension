...
from transformers import pipeline, AutoModelForQuestionAnswering, AutoTokenizer #from transformers import BertTokenizer, BertForQuestionAnswering
...
def qa_system(information, question):
    ...
    modelname = "medical_trained_model"  
    loaded_model = AutoModelForQuestionAnswering.from_pretrained(modelname)
    tokenizer = AutoTokenizer.from_pretrained(modelname)
    qa = pipeline("question-answering", model=loaded_model, tokenizer=tokenizer)
    ...
    ans1 = nlp1(ans['answer'])
    ans2 = nlp2(ans['answer'])
    ans3 = med7(ans['answer'])

    answers_entities = [ans1, ans2, ans3] 

    #answer_unique = answer_unique_function(docs, ans)
    #text = answer_unique.text
    #label = answer_unique.label_

    html_answers = render_text(answers_entities, models)
    
    file = open("./qa_system/templates/qa_system/answer1.html","w")
    file.write(html_answers[0])
    file.close()

    file = open("./qa_system/templates/qa_system/answer2.html","w")
    file.write(html_answers[1])
    file.close()

    file = open("./qa_system/templates/qa_system/answer3.html","w")
    file.write(html_answers[2])
    file.close()  

    html_context = render_text(docs, models)
    
    file = open("./qa_system/templates/qa_system/displacy1.html","w")
    file.write(html_context[0])
    file.close()

    file = open("./qa_system/templates/qa_system/displacy2.html","w")
    file.write(html_context[1])
    file.close()

    file = open("./qa_system/templates/qa_system/displacy3.html","w")
    file.write(html_context[2])
    file.close()      

    return {
        'full_answer': ans,
        'answer': ans["answer"],#text+' ['+label+']',
        'entities': entities,
...
def answer1_view(request):
    return render(request, "qa_system/answer1.html") 
def answer2_view(request):
    return render(request, "qa_system/answer2.html")     
def answer3_view(request):
    return render(request, "qa_system/answer3.html")  
